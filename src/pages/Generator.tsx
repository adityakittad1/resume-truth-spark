  ...
  return (
    <PageTransition className="min-h-screen bg-background flex flex-col">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <img src={logo} alt="Resumate" className="h-20 md:h-28 rounded-lg shadow-md transition-transform duration-300 hover:scale-105" />
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 md:py-12 flex-1">
        {/* Step: Mode Selection */}
        {step === 'select' && <div className="text-center mb-12 animate-fade-in">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto mb-10 animate-fade-in-up delay-100" style={{
          opacity: 0
        }}>
        ...
        {/* Step: Upload Resume */}
        {step === 'upload' && <ResumeUploader onParsed={handleParsedResume} onBack={handleBackToSelect} />}

        {/* Step: Form (Start from Scratch) */}
        {step === 'form' && <ResumeForm initialData={resumeData} onSubmit={data => {
        setResumeData(data);
        setStep('preview');
      }} onBack={handleBackToSelect} />}

        {/* Step: Improve Flow */}
        {step === 'improve' && <ImproveResumeFlow initialData={resumeData} onComplete={data => {
        setResumeData(data);
        setStep('preview');
      }} onBack={() => setStep('upload')} />}

        {/* Step: Preview */}
        {step === 'preview' && <ResumePreview data={resumeData} onBack={() => setStep('improve')} onEdit={() => setStep('improve')} />}
      </main>
      ...
      <footer className="border-t bg-muted/30">
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">© 2026 Resumate. Built by Aditya Kittad.</p>
            <a href="https://www.linkedin.com/company/resumate-io/" target="_blank" rel="noopener noreferrer">
              <Button variant="outline" size="sm" className="gap-2 transition-all duration-300 hover:scale-105 hover:bg-[#0077B5]/10 hover:border-[#0077B5]/50 hover:text-[#0077B5]">
